...
\begin{equation}
W_h := \{ v \in L^2(\Omega)\, |\, v|_{K} \in C^k(K) \,\forall K \in \mathcal{T} \}.
\end{equation}

We can then state the discrete weak formulation on each element as follows. Find $\bld{u}_h \in W_h^m$ such that
\begin{equation}
