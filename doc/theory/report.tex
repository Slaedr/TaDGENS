...
\begin{equation}
\int_{\Omega_e} \frac{\partial\bld{u}}{\partial t}w\,d\mu + \int_{\Gamma_e} \bld{F}(\bld{u})\hat{\bld{n}}w \,ds - \int_{\Omega_e}\bld{F}(\bld{u})\nabla w \,d\mu = \bld{0} \quad \forall w \in W,\, \forall \,\Omega_e \in \mathcal{T}.
\label{wf}
\end{equation}
...
\begin{equation}
\int_{\Omega_e} \frac{\partial\bld{u}_h}{\partial t}B_i\,d\mu + \int_{\Gamma_e} \bld{F}(\bld{u}_h)\hat{\bld{n}}B_i \,ds - \int_{\Omega_e}\bld{F}(\bld{u}_h)\nabla B_i \,d\mu = \bld{0} \quad \forall i \in {1,2,...n},\, \forall \,\Omega_e \in \mathcal{T}
\end{equation}
where $n$ is the number of degrees of freedom in an element. 

We can write $\bld{u}_h = \sum_{j=1}^n \bld{u}_j B_j(\bld{x})$. Further, we introduce the numerical flux $bld{h}$ defined such that for a face $f$ between two elements $\Omega_L$ and $\Omega_R$ with states (conserved variables) $\bld{u}_L$ and $ \bld{u}_R$,
\begin{equation}
...
The discrete form becomes: find $\bld{u}_h \in W_h^m$ such that
\begin{equation}
\sum_{i=1}^n\int_{\Omega_e} B_iB_j\,d\mu\, \frac{d\bld{u}_j}{d t} + \int_{\Gamma_e}  \bld{h}(\bld{u}_L, \bld{u}_R, \hat{\bld{n}})B_i \,ds - \int_{\Omega_e}\bld{F}(\bld{u}_h)\nabla B_i \,d\mu = \bld{0} \quad \forall i \in {1,2,...n},\, \forall \,\Omega_e \in \mathcal{T}.
\label{df}
\end{equation}
We thus need to solve the following ODE.
\begin{equation}
M \frac{d\bld{u}_h}{dt} = \bld{R}(\bld{u}_h(t)).
\label{ode}
\end{equation}
where $[M]_{ij} = \int_{\Omega_e} B_iB_j\,d\mu$ and $\bld{R}(\bld{u}_h) :=  - \int_{\Gamma_e}  \bld{h}(\bld{u}_L, \bld{u}_R, \hat{\bld{n}})B_i \,ds + \int_{\Omega_e}\bld{F}(\bld{u}_h)\nabla B_i \,d\mu$.

We denote by $\bld{U} := [\bld{u}_1\, \bld{u}_2\, ...\, \bld{u}_n]^T \in \mathbb{R}^{mn}$ the stacked vector of all degrees of freedom of all conserved variables on the element $\Omega_e$.
