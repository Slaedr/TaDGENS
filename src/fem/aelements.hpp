...
protected:
	...
	/// Magnitude of tangent vectors to the curve at quadrature points
	std::vector<a_real> speeds;

	/// Physical coordinates of the quadrature points
	/// ie, the mapping evaluated at the quadrature points
	Matrix mapping;
...
public:
	...
	/// Sets the polynomial degree, coordinates of physical nodes and the quadrature context
 ...
 * \f]
 * where \f$ \hat{t} \f$ is the unit tangent vector and \f$ \hat{k} \f$ is the "upward" unit vector
 * normal to the 2D domain.
 * If the nodes of the boundary (in intfac) are oriented properly
 * (which would be the order they would appear in phyNodes as well),
...
public:
	...
	/// Allocates and sets the Jacobian inverses and Jacobian determinants at the quadrature points
	 ...
	 * not for the Jacobian matrix or its inverse.
	 */
	...
	virtual void calculateJacobianDetAndInverse(const Matrix& refpoints,
	                                            std::vector<MatrixDim>& jacoi,
	                                            std::vector<a_real>& jacod) const = 0;

	/// Calculate physical coords of domain quadrature points separately
	/** In case needed by referential elements
	 */
...
public:
	...
	void calculateJacobianDetAndInverse(const Matrix& po, std::vector<MatrixDim>& jacoi,
	                                    std::vector<a_real>& jacod) const;
...
/** \todo Note that this implementation is not very good for elements with basis functions defined
 * on the reference element. Such elements would only need to store 1 set of basis function values
 * for all elements of a given polynomial order.
 * In contrast, elements defined in physical coordinates need to hold different basis function values
 * for different elements. So a better option is to leave storage of basis and basis gradients to
 * child classes and have pure virtual accessors here.
 */
class Element
{
protected:
	...
	/// Values of derivatives of the basis functions at the quadrature points
	std::vector<Matrix> basisGrad;
	/// The 2D geometric map which maps this element to the reference element
	const GeomMapping2D* gmap;
	/// This can be used to store basis and basis gradient values too
	const BasisSet* bset;							
...
public:
	...
	virtual void computeBasisGrads(const Matrix& points, const std::vector<MatrixDim>& jinv,
	                               std::vector<Matrix>& basisgrads) const = 0;
	 ...
	 * \param[in|out] values Values of the function at each quadrature point.
	 *   Each row contains all DOFs of a physical variable
	 *   CANNOT be the same as dofs, the first argument.
	 *   
	 * \deprecated in favor of [this global function](@ref evaluateFunctions)
	 */
	//[[deprecated(" in favor of evaluateFunctions")]]
	 ...
	 */
	void interpolateComponent(const int comp, const Matrix& __restrict__ dofs,
	                          Vector& __restrict__ values) const
	{
 ...
 *
 * Note that the first (p0) DOF is not the value at the element center, but the average value
 * over the element. We thus need to compute offsets from Taylor polynomials for terms associated
 * with P2 and higher.
 *
 * Computation of basis gradients does not need the Jacobian of the geometric mapping.
 * They are simply computed as
 * \f[ \nabla B(x) = \nabla B(F(\xi)) \f]
 */
class TaylorElement : public Element
{
	...
	/// The quantities by which the basis functions are offset from actual Taylor polynomial basis
	std::vector<std::vector<a_real>> basisOffset;
...
public:
	 ...
	 */
	void computeBasisGrads(const Matrix& points, const std::vector<MatrixDim>& jinv,
	                       std::vector<Matrix>& basisgrads) const;
 ...
 * \f]
 *
 * \todo TODO: Use BasisSet instead of storing basis and gradients at quadrature points for
...
protected:
	...
	/// Local face number of this face w.r.t the left and right elements
	int llfn, rlfn;

	/// Values of the left element's basis functions at the face quadrature points
	Matrix leftbasis;
	/// Values of the left element's basis functions at the face quadrature points
	Matrix rightbasis;

	/// left element's basis gradients at face quadrature points
	std::vector<Matrix> leftbgrad;
	/// right element's basis gradients at face quadrature points
	std::vector<Matrix> rightbgrad;

	/// 1D geometric mapping (parameterization) of the face
	const GeomMapping1D* gmap;							

	/// Computes 2D reference coordinates of 1D face reference points
	///  on the face of an element that shares this face
	/** \param[in] facepoints 1D coordinates on the face
	 ...
	 * \praram[in|out] lpoints Contains 2D reference coordinates of face quadrature points in
	 *   element elem on output.
	 * \return Returns the magnitude of the speed of the face of the reference element w.r.t. the
	 *   reference face.
	 */
...
public:
	...
	/** \note Call only after element data has been precomputed, ie, by calling the compute function
	 *   on the elements, first!
	 * \param[in] geommap The geometric mapping must be [initialized](@ref GeomMapping1D::setAll)
	 *   and map and normals [computed externally](@ref GeomMapping1D::computeAll)
	 ...
	 */
	void initialize(const Element *const lelem, const Element *const relem,
	                const GeomMapping1D *const geommap, const int l_localface, const int r_localface);
	...
	void interpolateAll_left(const Matrix& dofs, Matrix& __restrict__ values) {
		values.noalias() = leftbasis*dofs.transpose();
	}

	void interpolateAll_right(const Matrix& dofs, Matrix& __restrict__ values) {
 ...
 */
inline void evaluateFunctions(const Matrix& dofs, const Matrix& basisv,
                              Matrix& __restrict__ interp)
{
	interp.noalias() = basisv * dofs.transpose();
}

/// Evaluate values of gradients of a function using BLAS 3 calls for several points at once
/** \param[in] dofs Array of DOFs - each row contains the DOFs of one physical variable (nvars x ndofs)
 * \param[in] basisg Set of arrays of basis gradient values at the points at which function values
 *  are needed; each row has the gradient corresponding to a given dof (npoints x (ndofs x ndim))
 * \param[in|out] interp Pre-allocated set of arrays for storing interpolated values
 *   (npoints x (nvars x ndim))
 */
template <typename T>
inline void evaluateGradients(const Matrix& dofs,
                              const std::vector<Matrix>& basisg,
                              std::vector<T>& __restrict__ interp)
{
	for(size_t ig = 0; ig < basisg.size(); ig++)
		interp[ig].noalias() = dofs * basisg[ig];
}

void getTaylorBasis(const Matrix& gp, const int degree,
                    const a_real *const center, const a_real *const delta,
                    const std::vector<std::vector<a_real>>& basisOffset,
                    Matrix& __restrict__ basiss);

void getTaylorBasisGrads(const Matrix& gp, const int degree,
                         const a_real *const center, const a_real *const delta,
